...
// INDIVIDUAL STOCK ARRAY API CALL
// S0_500.map((stock, index) => {
//   setTimeout(() => APIrequest(stock, `${api_keys[0].api_key}`), index * 30000);
// });

// S501_1000.map((stock, index) => {
//   setTimeout(() => APIrequest(stock, `${api_keys[1].api_key}`), index * 30000);
// });

// S1001_1500.map((stock, index) => {
//   setTimeout(() => APIrequest(stock, `${api_keys[2].api_key}`), index * 30000);
// });

// S1501_2000.map((stock, index) => {
//   setTimeout(() => APIrequest(stock, `${api_keys[3].api_key}`), index * 30000);
// });

app.get("/", (req, res) => {
  let count = 0;
  const reqInterval = setInterval(() => {
    APIrequest(S0_500[count], `${api_keys[0].api_key}`);
    // APIrequest(
    //   S501_1000[count],
    //   `${api_keys[1].api_key}`,
...
request("http://127.0.0.1:4000", (req, res) => console.log("oupa"));
// Notes
